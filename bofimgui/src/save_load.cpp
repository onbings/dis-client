//NODE
#include "bofimgui/imnodes_editor.h"
#include "bofimgui/imnodes_graph.h"
...
namespace
{
  ...
  struct Node
  {
    ...
    Node(const int i, const float v) : id(i), type(NodeType::add), value(v)
    {
    }


    explicit Node(const NodeType t) : id(0), type(t), value(0.f)
    {
    }
    ...
    void show()
    {
      ...
      // These are driven by the user, so we place this code before rendering the nodes
      {
        const bool open_popup = ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) &&
          ...
          ImGui::IsKeyReleased(ImGuiKey_A); //BHA
...

      {
        ...
        if (num_selected > 0 && ImGui::IsKeyReleased(ImGuiKey_X)) //BHA
        {
...
public:
    ...
    void show()
    {
        ...
        if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows))
        {
          ...
          {
            if (ImGui::IsKeyReleased(ImGuiKey_A)) //BHA
            {
